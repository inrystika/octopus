...
type OctopusError struct {
	CodeType    CodeType
	Code        int
	Message     string
	stdError    error
	prevErr     *OctopusError // prevErr 指向上一个Err
	stack       []uintptr
	once        sync.Once
	fullMessage string
}
...
func (e *OctopusError) HTTPCode() int {
	if e == nil {
		return int(Unknown)
	}
...
func (e *OctopusError) HTTPSubCode() int {
	if e == nil {
		return ErrorUnknown
	}
...
func (e *OctopusError) HTTPErrMsg() string {
	if e == nil {
...
func (e *OctopusError) Error() string {
	e.once.Do(func() {
...
//如果参数error类型不为*OctopusError(error常量或自定义error类型或nil), 用于最早出错的地方, 会收集调用栈
//如果参数error类型为*OctopusError, 不会收集调用栈
func Errorf(err error, code int) error {
	if err, ok := err.(*OctopusError); ok {
		return &OctopusError{
...
func new_(code int, codeMsg codeMsg) *OctopusError {
	pc := make([]uintptr, 200)
...
func FromError(err error) (*OctopusError, bool) {
	if se := new(OctopusError); errors.As(err, &se) {
		return se, true
	}
	return nil, false
}
