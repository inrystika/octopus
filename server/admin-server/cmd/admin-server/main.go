...
import (
	...
	"server/admin-server/internal/server"
...
func initApp(ctx context.Context, bc *conf.Bootstrap, logger log.Logger) (*kratos.App, error) {
	data, err := data.NewData(bc.Data, logger)
	if err != nil {
		return nil, err
	}

	service := service.NewService(bc, logger, data)
...
func initConf() (*conf.Bootstrap, config.Config, error) {
	...
	// init Secrect
	conf.Server.Http.JwtSecrect = jwt.FormatToken(constant.SYSTEM_TYPE_ADMIN, conf.Server.Http.JwtSecrect)
	err := c.Watch("server.http.jwtSecrect", func(s string, value config.Value) {
		...
		conf.Server.Http.JwtSecrect = jwt.FormatToken(constant.SYSTEM_TYPE_ADMIN, val)
	})
	if err != nil {
