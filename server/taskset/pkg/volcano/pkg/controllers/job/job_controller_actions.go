...
func (cc *jobcontroller) syncJob(jobInfo *apis.JobInfo, updateStatus state.UpdateStatusFn) error {
	...
	for idx, ts := range job.Spec.Tasks {
		...
		for i := 0; i < int(ts.Replicas); i++ {
			...
			if pod, found := pods[podName]; !found {
				...
				delete(pods, podName)
				//updatePodInfo(&taskRoleStatus[idx].ReplicaStatuses[i], pod)
				if pod.DeletionTimestamp != nil {
					klog.Infof("Pod <%s/%s> is terminating", pod.Namespace, pod.Name)
	...
	job.Status = batch.JobStatus{
		...
		StartAt:             job.Status.StartAt,
		FinishAt:            job.Status.FinishAt,
		ToEvict:             job.Status.ToEvict,
...
func (cc *jobcontroller) createJobIOIfNotExist(job *batch.Job) (*batch.Job, error) {
	...
	if job.Spec.PrivilegeJob {
		job.Annotations[schedulingv2.PrivilegeJobAnnotationKey] = schedulingv2.PrivilegeJobAnnotationValue
	}
