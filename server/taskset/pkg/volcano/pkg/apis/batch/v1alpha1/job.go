...
type JobSpec struct {
	...
	// If true, this job will preempt a job whose priority is lower in the same queue when the resource is in lack. 
	PrivilegeJob bool `json:"privilegeJob,omitempty" protobuf:"bytes,11,opt,name=privilegeJob"`
}
...
type JobStatus struct {
	...
	ToEvict	bool  `json:"toEvict,omitempty" protobuf:"bytes,16,opt,name=toEvict"`   //将被驱逐

	Privileger string `json:"privileger,omitempty" protobuf:"bytes,17,opt,name=Privileger"`   //被谁驱逐
}
...
type JobInfo struct {
	PodRoleName map[string]string		`json:"podRoleName"`
	PodEvents 	map[string][]*PodEvent	`json:"podEvents"`
	Extras		[]*PodEvent				`json:"extras"`
}