...
func NewTaskInfo(pod *v1.Pod) *TaskInfo {
	...
	if pod.Annotations[v1beta1.WillEvictAnnotationKey] == v1beta1.WillEvictAnnotationValue {
		ti.WillEvict = true
		ti.EvictBy = TaskID(pod.Annotations[v1beta1.EvictByAnnotationKey])
	}
	ti.PriNodeName = pod.Annotations[v1beta1.PriNodeNameAnnotationKey] 
	if val, found := pod.Annotations[v1beta1.EvictsAnnotationKey]; found{
		if val != "" {
			json.Unmarshal([]byte(val), &ti.Evicts)
		}
	} 
	return ti
}
...
// AddEvict .
func (ti *TaskInfo) IsEvictor() bool {
	if ti.Evicts == nil {
		return false
	}
	return len(ti.Evicts) > 0
} 

// AddEvict .
func (ti *TaskInfo) AddEvict(t *TaskInfo) {
	found := false
	tid := PodKey(t.Pod)
	if ti.Evicts == nil {
		ti.Evicts = []TaskID{}
	}
	for _, v := range ti.Evicts {
		if v == tid {
			found = true
			break
		}
	...
	if !found {
		ti.Evicts = append(ti.Evicts, tid)
	}
}

// RemoveEvict .
func (ti *TaskInfo) RemoveEvict(t *TaskInfo) {
	idx := -1
	tid := PodKey(t.Pod)
	for i, v := range ti.Evicts {
		if v == tid {
			idx = i
			break
		}
	...
	if idx >= 0 {
		ti.Evicts = append(ti.Evicts[:idx], ti.Evicts[idx+1:]...)
	}
}
...
// IsPrivilegeJob is used to judge  ...
func (ji *JobInfo) IsPrivilegeJob() bool {
	if val, found := ji.PodGroup.Annotations[v1beta1.PrivilegeJobAnnotationKey]; found && val == v1beta1.PrivilegeJobAnnotationValue {
		return true
	}
	return false
}

// HasPrivileged is used to judge  ...
func (ji *JobInfo) HasPrivileged() bool {
	if val, found := ji.PodGroup.Annotations[v1beta1.HasPrivilegedAnnotationKey]; found && val == v1beta1.HasPrivilegedAnnotationValue {
		return true
	}
	return false
}
...
// SetPrivileged  ...
func (ji *JobInfo) SetPrivileged() {
	ji.PodGroup.Annotations[v1beta1.HasPrivilegedAnnotationKey] = v1beta1.HasPrivilegedAnnotationValue
}

// SetTaskToEvict  ...
func (ji *JobInfo) SetTaskToEvict(task *TaskInfo, toEvict bool, evictBy *TaskInfo) {
	if t, found := ji.Tasks[task.UID]; found {
		t.WillEvict = toEvict
		if toEvict {
			t.EvictBy = PodKey(evictBy.Pod)
		}else{
			t.EvictBy = ""
		}
	}
...
// SetTaskToEvict  ...
func (ji *JobInfo) SetEvictTask(task *TaskInfo, toEvict bool, evictBy *TaskInfo) {
	if t, found := ji.Tasks[evictBy.UID]; found {
		if toEvict {
			t.AddEvict(task)
		}else{
			t.RemoveEvict(task)
...
// SetTaskPrivilegedNode  ...
func (ji *JobInfo) SetTaskPrivilegedNode(task *TaskInfo, priNodeName string) {
	if t, found := ji.Tasks[task.UID]; found {
		t.PriNodeName = priNodeName
	}
}
...
// PrivilegedTaskNum returns the number of tasks that are Privileged.
func (ji *JobInfo) PrivilegedTaskNum() int32 {
	occupid := 0
	for status, tasks := range ji.TaskStatusIndex {
...
// Privileged .
func (ji *JobInfo) Privileged() bool {
	occupied := ji.PrivilegedTaskNum() + ji.ReadyTaskNum()
	return occupied >= ji.MinAvailable
}