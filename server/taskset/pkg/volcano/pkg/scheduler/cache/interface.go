...
type Cache interface {
	...
	// LableJobEvict will label the job to evict.
	LableJobEvict(p *api.TaskInfo, taskInfo *api.TaskInfo, toEvict bool, reason string, needLock bool) error

	// Privilege .
	Privilege(privileger *api.TaskInfo, privilege bool) error

	// GetNodeWillEvictTag .
	GetNodeWillEvictTag(nodeName string) (*api.Resource, error)

	// GetNodeWillEvictTasksTag .
	GetNodeWillEvictTasksTag(nodeName string) (map[api.TaskID]*api.EvictTaskTagInfo, error)

	// ResetWillEvictTag .
	ResetWillEvictTag(privileger *api.TaskInfo, nodeName string) error
	...
	// UpdateNamspacePriority .
	UpdateNamspacePriority(namespace string, priority int32) 
...
type Evictor interface {
	...
	LableJobEvict(j *api.JobInfo, t *api.TaskInfo, privileger *api.TaskInfo, toEvict bool, reason string) error
}
