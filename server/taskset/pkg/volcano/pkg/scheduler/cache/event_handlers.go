...
func (sc *SchedulerCache) addTask(pi *schedulingapi.TaskInfo) error {
	...
	if len(pi.PriNodeName) != 0 {
		if _, found := sc.Nodes[pi.PriNodeName]; !found {
			sc.Nodes[pi.PriNodeName] = schedulingapi.NewNodeInfo(nil)
		}
		node := sc.Nodes[pi.PriNodeName]
		if !isTerminated(pi.Status) {
			for _, taskKey := range pi.Evicts {
				task, found := node.Tasks[taskKey]
				if found {
					sc.LableJobEvict(pi, task, true, "", false)
				}
			}
			return node.AddPrivilegeTask(pi)
		}
	}
...
func (sc *SchedulerCache) updatePod(oldPod, newPod *v1.Pod) error {
	willEvict := false
	var evictBy schedulingapi.TaskID = ""
	priNodeName := ""
	evicts := ""
	pi := schedulingapi.NewTaskInfo(oldPod)
	task := pi
	...
	if task != nil {
		willEvict = task.WillEvict
		evictBy = task.EvictBy
		priNodeName = task.PriNodeName
	}
	if priNodeName != "" {
		bytes, _ := json.Marshal(task.Evicts)
		evicts = string(bytes)
	}
	...
	if willEvict {
		newPod.Annotations[schedulingv1.WillEvictAnnotationKey] = schedulingv1.WillEvictAnnotationValue
		newPod.Annotations[schedulingv1.EvictByAnnotationKey] = string(evictBy)
	}
	newPod.Annotations[schedulingv1.PriNodeNameAnnotationKey] = priNodeName
	newPod.Annotations[schedulingv1.EvictsAnnotationKey] = evicts
...
func (sc *SchedulerCache) deleteTask(pi *schedulingapi.TaskInfo) error {
	...
	if len(pi.PriNodeName) != 0 {
		node := sc.Nodes[pi.PriNodeName]
		if node != nil {
			node.ResetWillEvictTag(pi)
			node.RemovePrivilegeTask(pi)
			ts := node.GetEvictTaskByPrivilege(schedulingapi.PodKey(pi.Pod))
			for _, t := range ts {
				sc.LableJobEvict(pi, t, false, "", false)
			}
		}
	}
