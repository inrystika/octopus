...
type queueAttr struct {
	...
	inqueueJob map[api.JobID]api.JobID
}
...
func (pp *proportionPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	for _, job := range ssn.Jobs {
		...
		if _, found := pp.queueOpts[job.Queue]; !found {
			...
			attr := &queueAttr{
				...
				inqueueJob: make(map[api.JobID]api.JobID),
			}
		...
		} else if job.PodGroup.Status.Phase == scheduling.PodGroupPrivilege && job.IsPrivilegeJob() && job.HasPrivileged() {
			attr.inqueue.Add(GetJobMinResources(job.PodGroup.Spec))
		}
	}
	...
	ssn.AddJobEnqueueableFn(pp.Name(), func(obj interface{}) bool {
		...
		inqueue := false
		if _, found := attr.inqueueJob[job.UID]; !found {
			var minReq *api.Resource = nil
			if job.PodGroup.Status.Phase == scheduling.PodGroupInqueue {
				minReq = api.EmptyResource()
			} else if job.PodGroup.Status.Phase == scheduling.PodGroupPrivilege && job.IsPrivilegeJob() && job.HasPrivileged() {
				minReq = api.EmptyResource()
			} else {
			...
			if inqueue {
				attr.inqueueJob[job.UID] = job.UID
				attr.inqueue.Add(GetJobMinResources(job.PodGroup.Spec))
			}
		} else {
			inqueue = true
	...
	ssn.AddPrivilegeJobEnqueueableFn(pp.Name(), func(obj interface{}) bool {
		job := obj.(*api.JobInfo)
		queueID := job.Queue
		attr := pp.queueOpts[queueID]
		queue := ssn.Queues[queueID]
		...
		taskUsed := api.EmptyResource()
		for _, node := range allNodes {
			nodeMatched := false
			for qk, qv := range queue.Queue.Spec.NodeSelector {
			...
			for _, task := range node.Tasks {
				...
				cmpjob, found := ssn.Jobs[task.Job]
				if !found {
					continue
				}
				if job.Queue != cmpjob.Queue || job.UID == cmpjob.UID {
					continue
				}
				prvilegees = append(prvilegees, task.Clone())
			}
			for _, privileger := range job.TaskStatusIndex[api.Pending] {
				candidates := ssn.Preemptable(privileger, prvilegees)
		...
		if _, found := attr.inqueueJob[job.UID]; found || (job.PodGroup.Status.Phase == scheduling.PodGroupInqueue) {
			minReq = api.EmptyResource()
		} else if job.PodGroup.Status.Phase == scheduling.PodGroupPrivilege && job.IsPrivilegeJob() && job.HasPrivileged() {
			minReq = api.EmptyResource()
		}

		inqueue := minReq.Add(attr.allocated).Add(attr.inqueue).Sub(taskUsed).LessEqual(api.NewResource(queue.Queue.Spec.Capability))

		return inqueue
	})

	ssn.AddCurPrivilegeJobEnqueueableFn(pp.Name(), func(obj interface{}) bool {
		job := obj.(*api.JobInfo)
		queueID := job.Queue
		attr := pp.queueOpts[queueID]
		queue := ssn.Queues[queueID]
		...
		allNodes := util.GetNodeList(ssn.Nodes)
		taskWillEvict := api.EmptyResource()
		for _, node := range allNodes {
			nodeMatched := false
			for qk, qv := range queue.Queue.Spec.NodeSelector {
				for k, v := range node.Node.Labels {
		...
		inqueue := minReq.Add(attr.allocated).Add(attr.inqueue).Sub(taskWillEvict).LessEqual(api.NewResource(queue.Queue.Spec.Capability))
