...
// SelectPrivilegedNode ..
func SelectPrivilegedNode(p *api.TaskInfo, nodes []*api.NodeInfo) *api.NodeInfo {

	var ret *api.NodeInfo = nil
	for _, node := range nodes {
		if node.Name == p.PriNodeName {
			ret = node
			break
		}else if node.Name == p.NodeName {
			ret = node
			break
		}
	}
...
// ValidatePrivilegeVictims returns an error if the resources of the victims can't satisfy the preemptor
func ValidatePrivilegeVictims(preemptor *api.TaskInfo, node *api.NodeInfo, victims []*api.TaskInfo) error {
	if len(victims) == 0 {
		return fmt.Errorf("no victims")
	}
	futureIdle := node.Idle.Clone()
	for _, victim := range victims {
		futureIdle.Add(victim.Resreq)
	}
	futureIdle.Add(node.WillEvict).Sub(node.Privileged)
	// Every resource of the preemptor needs to be less or equal than corresponding
	// idle resource after preemption.
	if !preemptor.InitResreq.LessEqual(futureIdle) {
		return fmt.Errorf("not enough resources: requested <%v>, but future idle <%v>",
