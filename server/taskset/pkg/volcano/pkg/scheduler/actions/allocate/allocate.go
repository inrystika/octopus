...
func (alloc *Action) Execute(ssn *framework.Session) {
	...
	predicateFn := func(task *api.TaskInfo, node *api.NodeInfo) error {
		...
		if job, found := ssn.Jobs[task.Job]; !found {
			return api.NewFitError(task, node, api.JobNotFoundMsg)
		}else{
			if job.IsPrivilegeJob() && job.HasPrivileged() && ssn.JobEnqueueable(job){
				if !task.InitResreq.LessEqual(node.Idle) || !task.InitResreq.LessEqual(node.PrivilegeIdleExceptOne(task)) {
					return api.NewFitError(task, node, api.NodeResourceFitFailed)
				}
			}else{
				if job.IsPrivilegeJob() && !ssn.JobEnqueueable(job){
					return api.NewFitError(task, node, api.NodeResourceFitFailed)
				}
				if !task.InitResreq.LessEqual(node.Idle) || !task.InitResreq.LessEqual(node.FutureIdle()) {
					return api.NewFitError(task, node, api.NodeResourceFitFailed)
				}
			}
		}
	...
	for {
		...
		for queueID := range queueInNamespace {
			...
			if _, found := pendingTasks[job.UID]; !found {
				pTasks := util.NewPriorityQueue(ssn.TaskOrderFn)
			...
			for !tasks.Empty() {
				...
				if job.IsPrivilegeJob() && job.HasPrivileged() && ssn.JobEnqueueable(job){
					for _, n := range predicateNodes {
						if task.InitResreq.LessEqual(n.Idle) && task.InitResreq.LessEqual(n.PrivilegeIdleExceptOne(task)) {
					...
					for _, n := range predicateNodes {
						if task.InitResreq.LessEqual(n.Idle) && (task.InitResreq.LessEqual(n.PrivilegeIdle()) || task.InitResreq.LessEqual(n.FutureIdle())) {
							candidateNodes = append(candidateNodes, n)
						}
					}
				}
				...
				if job.IsPrivilegeJob() && job.HasPrivileged(){
					node = util.SelectPrivilegedNode(task, candidateNodes)
				}else{
				...
				if job.IsPrivilegeJob() && job.HasPrivileged() && ssn.JobEnqueueable(job){
					...
					if task.InitResreq.LessEqual(node.Idle) && task.InitResreq.LessEqual(node.PrivilegeIdle()){
						bind = true
					}
				}
				...
				if bind {
					...
					if err := stmt.Allocate(task, node.Name); err != nil {
						...
						if job.IsPrivilegeJob() && job.HasPrivileged() {
							//stmt.ResetWillEvictTag(task)
							if ni, found := ssn.Nodes[task.PriNodeName]; found {
								for _, ti := range ni.WillEvictTasks {
									if err := stmt.CancelJobEvict(task, ti, ""); err != nil {
										klog.Errorf("Failed to cancelJobEvict Task <%s/%s> for Tasks <%s/%s>: %v",
											ti.Namespace, ti.Name, task.Namespace, task.Name, err)
										continue
									}
								}
								if err := stmt.CancelPrivilege(task, task.Status, task.PriNodeName); err != nil {
									klog.Errorf("Failed to CancelPrivilege Task <%s/%s> : %v", task.Namespace, task.Name, err)
									continue
								} 
							}
						}
					...
					if !job.IsPrivilegeJob() && task.InitResreq.LessEqual(node.Idle) && task.InitResreq.LessEqual(node.FutureIdle()) {
