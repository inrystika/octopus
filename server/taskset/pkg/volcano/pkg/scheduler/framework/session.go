...
type Session struct {
	...
	jobPrivilegedFns  map[string]api.ValidateFn
	jobValidFns       map[string]api.ValidateExFn
	jobEnqueueableFns map[string]api.ValidateFn
	privilegeJobEnqueueableFns map[string]api.ValidateFn
	curPrivilegeJobEnqueueableFns map[string]api.ValidateFn
}
...
func jobStatus(ssn *Session, jobInfo *api.JobInfo) scheduling.PodGroupStatus {
	...
	if len(jobInfo.TaskStatusIndex[api.Running]) != 0 && unschedulable {
		...
		} else if jobInfo.PodGroup.Status.Phase != scheduling.PodGroupInqueue && jobInfo.PodGroup.Status.Phase != scheduling.PodGroupPrivilege {
