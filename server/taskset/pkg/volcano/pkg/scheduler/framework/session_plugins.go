...
// AddJobPrivilegedFn add privileged function
func (ssn *Session) AddJobPrivilegedFn(name string, vf api.ValidateFn) {
	ssn.jobPrivilegedFns[name] = vf
}
...
// AddPrivilegeJobEnqueueableFn add privilegeJobEnqueueableFns function
func (ssn *Session) AddPrivilegeJobEnqueueableFn(name string, fn api.ValidateFn) {
	ssn.privilegeJobEnqueueableFns[name] = fn
}

// AddCurPrivilegeJobEnqueueableFn add curPrivilegeJobEnqueueableFns function
func (ssn *Session) AddCurPrivilegeJobEnqueueableFn(name string, fn api.ValidateFn) {
	ssn.curPrivilegeJobEnqueueableFns[name] = fn
}
...
// JobPrivileged invoke pipelined function of the plugins
func (ssn *Session) JobPrivileged(obj interface{}) bool {
	for _, tier := range ssn.Tiers {
		for _, plugin := range tier.Plugins {
			if !isEnabled(plugin.EnabledJobPrivileged) {
				continue
			}
			jrf, found := ssn.jobPrivilegedFns[plugin.Name]
			if !found {
				continue
			}
...
// PrivilegeJobEnqueueable invoke privilegeJobEnqueueableFns function of the plugins
func (ssn *Session) PrivilegeJobEnqueueable(obj interface{}) bool {
	for _, tier := range ssn.Tiers {
		for _, plugin := range tier.Plugins {
			fn, found := ssn.privilegeJobEnqueueableFns[plugin.Name]
			if !found {
				continue
			}
...
// CurPrivilegeJobEnqueueable invoke curPrivilegeJobEnqueueable function of the plugins
func (ssn *Session) CurPrivilegeJobEnqueueable(obj interface{}) bool {
	for _, tier := range ssn.Tiers {
		for _, plugin := range tier.Plugins {
			fn, found := ssn.curPrivilegeJobEnqueueableFns[plugin.Name]
			if !found {
				continue
			}
