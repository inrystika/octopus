...
func (k *kubeImp) Create(ts *typeJob.TaskSet) error {
	if nil == ts {
...
func (k *kubeImp) processNewTaskSet(ts *typeJob.Job) {
...
func (k *kubeImp) processEvent() {
	...
	for _, tss := range k.tasksets {
		...
		for _, ts := range tss {
			if ts.taskset.Status.Phase == typeJob.Completed {
				continue
			}
			ts.counted++
