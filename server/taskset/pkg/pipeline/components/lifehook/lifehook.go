...
func (rl *HookTaskStateRequestLinker) next(focus TaskStatesProgressValue) *HookTaskRequest {
	...
	if r, ok := rl.requests.Load(TaskStatesProgressSuspended); ok {
		...
		// handle fast failed
		_, FailedOk := rl.requests.Load(TaskStatesProgressFailed)
		_, RunningOk := rl.requests.Load(TaskStatesProgressRunning)
		if FailedOk && !RunningOk {
			rl.setProgress(TaskStatesProgressFailed)
			cp = TaskStatesProgressFailed
		}
