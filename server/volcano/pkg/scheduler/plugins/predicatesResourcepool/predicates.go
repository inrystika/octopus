...
package predicatesResourcepool
...
import (
	...
	typeFw "server/volcano/pkg/scheduler/framework"
...
const (
	...
	PluginName = "predicatesResourcepool"
...
func New(arguments framework.Arguments) typeFw.Plugin {
...
func (pp *predicatesPlugin) OnSessionOpen(ssn *framework.Session, typeSsn *typeFw.Session) {
	...
	ssn.AddPredicateFn(pp.Name(), func(task *api.TaskInfo, node *api.NodeInfo) error {
		...
		predicateByStablefilter := func(pod *v1.Pod, nodeInfo *k8sframework.NodeInfo) (bool, error) {
			...
			// CheckNodeSelector
			nodeSelectorMatched := false
			if j, found := ssn.Jobs[task.Job]; !found {
				return false, api.NewFitError(task, node, typeApi.NodeSelectorFitFailed)
			} else {
				q := typeSsn.TypeQueues[j.Queue].Queue
				for qk, qv := range q.Spec.NodeSelector {
				...
				if !nodeSelectorMatched {
					if q.Name == "default" {
						rPoolBindingNodeLabelKey := fmt.Sprintf(typeApi.PoolBindingNodeLabelKeyFormat, typeApi.DefaultPoolName)
						rPoolBindingNodeLabelValue := typeApi.PoolBindingNodeLabelValue
						for k, v := range node.Node.Labels {
...
func (pp *predicatesPlugin) OnSessionClose(ssn *framework.Session, typeSsn *typeFw.Session) {}
