	...
	ssn.AddPredicateFn(pp.Name(), func(task *api.TaskInfo, node *api.NodeInfo) error {
		...
		predicateByStablefilter := func(pod *v1.Pod, nodeInfo *k8sframework.NodeInfo) (bool, error) {
			...
			if j, found := ssn.Jobs[task.Job]; !found {
				return false, api.NewFitError(task, node, api.NodeSelectorFitFailed)
			} else {
				q := ssn.Queues[j.Queue].Queue
				...
				if !nodeSelectorMatched {
					if q.Name == "default" {
						rPoolBindingNodeLabelKey := fmt.Sprintf(PoolBindingNodeLabelKeyFormat, DefaultPoolName)
						rPoolBindingNodeLabelValue := PoolBindingNodeLabelValue
						for k, v := range node.Node.Labels {
