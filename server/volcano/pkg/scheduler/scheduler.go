...
func NewScheduler(
	...
	scheduler := &Scheduler{
		...
		fileWatcher:    watcher,
		cache:          vcCache.New(config, schedulerName, defaultQueue, nodeSelectors),
		queueCache:     typeCache.New(config),
...
func (pc *Scheduler) Run(stopCh <-chan struct{}) {
	...
	go pc.queueCache.Run(stopCh)
	pc.queueCache.WaitForCacheSync(stopCh)
...
func (pc *Scheduler) runOnce() {
	...
	ssn, typeSsn := typeFw.OpenSession(pc.cache, pc.queueCache, plugins, configurations)
	defer typeFw.CloseSession(ssn, typeSsn)
