...
type PlatformTrainJob struct {
	...
	DatasetName              string     `gorm:"type:varchar(100);comment:'数据集名称'"`
	DatasetVersion           string     `gorm:"type:varchar(100);comment:'数据集版本'"`
	DatasetAddr              string     `gorm:"type:varchar(200);comment:'数据集地址'"`
	DatasetStorageConfigName string     `gorm:"type:varchar(100);comment:'数据集存储配置名'"`
