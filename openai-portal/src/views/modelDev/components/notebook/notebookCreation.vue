    ...
    <el-dialog title="创建NoteBook" width="55%" :visible.sync="CreateFormVisible" :before-close="handleDialogClose"
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" :label-width="formLabelWidth" class="demo-ruleForm">
        ...
        <el-form-item label="描述" :label-width="formLabelWidth" prop="desc">
          <el-input :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入notebook描述" maxlength="300" show-word-limit
        ...
        <div>
          ...
          <el-form-item label="算法版本" prop="algorithmVersion" v-if="algorithmVersion" style="display:inline-block;">
            <el-select v-model="ruleForm.algorithmVersion" placeholder="请选择算法版本" v-loadmore='loadAlgorithmVersion'>
              <el-option v-for="item in algorithmVersionOption"
                :key="item.algorithmDetail.algorithmId+item.algorithmDetail.algorithmVersion"
                :label="item.algorithmDetail.algorithmVersion" :value='item.algorithmDetail.algorithmVersion'>
              </el-option>
            </el-select>
          </el-form-item>
        </div>
        ...
        <div>
          ...
          <el-form-item label="数据集名称" prop="dataSetId" v-if="dataSetName" style="display: inline-block;">
            <el-select v-model="ruleForm.dataSetId" placeholder="请选择数据集名称" v-loadmore='loadDataSetName'
              ...
              <el-option v-for="item in dataSetNameOption" :key="item.id+item.name" :label="item.name" :value='item.id'>
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="数据集版本" prop="dataSetVersion" v-if="dataSetVersion" style="display: inline-block;">
            <el-select v-model="ruleForm.dataSetVersion" placeholder="请选择数据集版本" v-loadmore='loadDataSetVersion'>
              <el-option v-for="item in dataSetVersionOption" :key="item.datasetId+item.version" :label="item.version"
                :value='item.version'>
              </el-option>
            </el-select>
          </el-form-item>
        ...
        <el-form-item label="资源规格" :label-width="formLabelWidth" prop="specification">
          <el-select v-model="ruleForm.specification" placeholder="请资源规格" style="width:35%">
            <el-option v-for="(item, index) in resourceList" :key="index" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    methods: {
      ...
      getImageNameList(searchKey) {
        if (this.ruleForm.imageSource === 'my') {
          getMyImage({ pageIndex: this.imageCount, pageSize: 10, imageStatus: 3, imageType: 1, searchKey: searchKey }).then(response => {
            if (searchKey && searchKey.length != 0) {
        ...
        if (this.ruleForm.imageSource === 'common') {
          getPublicImage({ pageIndex: this.imageCount, pageSize: 10, imageStatus: 3, imageType: 1, searchKey: searchKey }).then(response => {
            if (searchKey && searchKey.length != 0) {
              this.imageNameOption = response.data.images
            } else {
              if (response.data.images.length !== 0) {
      ...
      remoteImage(searchKey) {
        this.getImageNameList(searchKey)
      },
      //远程请求数据集名称
