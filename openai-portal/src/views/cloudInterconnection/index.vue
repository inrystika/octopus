  ...
  <div>
    <el-tabs class="Wrapper">
      <el-tab-pane label="训练任务">
        ...
        <el-table :data="trainJobList" style="width: 100%;font-size: 15px;"
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button v-if="({'0':true,'1':true,'2':true,'7':true})[scope.row.status] || false" type="text"
                slot="reference" @click="confirmStop(scope.row)">
                停止
              </el-button>
            </template>
          </el-table-column>
    ...
    <trainingTaskCreate v-if="createVisible" @cancel="cancel" @confirm="confirm" @close="close" />
  </div>
</template>
<script>
